...
type App() as this = 
    inherit Application()
    do Application.LoadComponent(this, new System.Uri("/Samples.SourceWindow;component/App.xaml", System.UriKind.Relative));
    let sourceWindow = new Samples.SourceWindow.SourceWindowControl()
    do this.Startup.Add (fun _ -> 
            this.RootVisual <- sourceWindow
            // Make the control available to JavaScript code
            HtmlPage.RegisterScriptableObject("SourceWindow", sourceWindow)
        )
